...
function useAudio(enabled: boolean) {
  ...
  useEffect(() => {
    ...
    // Browsers suspend AudioContext until a direct user gesture.
    // Resuming inside a setTimeout doesn't count — we must resume
    // synchronously from within a real event handler.
  ...
  const playSound = (sound: [number, number] | undefined) => {
    if (!readyRef.current || !ctxRef.current || !bufferRef.current || !sound)
      return;
    // Attempt resume as a fallback (works if context was never suspended)
...
export function Terminal({
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "mx-auto w-full max-w-full px-4 font-mono text-xs",
        className,
      )}
    >
      <div className="overflow-hidden rounded-lg border border-neutral-800 bg-neutral-900 shadow-2xl">
        ...
        <div
          ref={contentRef}
          className="no-visible-scrollbar h-[520px] overflow-y-auto p-4 font-mono scrollbar-thin scrollbar-thumb-neutral-700 scrollbar-track-transparent"
        >
          {lines.map((line, i) => (
